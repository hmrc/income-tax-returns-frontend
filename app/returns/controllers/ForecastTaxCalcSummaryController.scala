...
class ForecastTaxCalcSummaryController @Inject()(val authActions: AuthActions,
  ...
  def backUrl(isAgent: Boolean, taxYear: Int, origin: Option[String]): String =
    if (isAgent) returns.controllers.routes.TaxYearSummaryController.renderAgentTaxYearSummaryPage(taxYear).url
    else returns.controllers.routes.TaxYearSummaryController.renderTaxYearSummaryPage(taxYear, origin).url
}
